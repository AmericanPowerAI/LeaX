...
# Plan pricing
PLANS = {
    'basic': {'price': 29.99, 'name': 'Basic Plan'},
    'standard': {'price': 59.99, 'name': 'Standard Plan'},
    'enterprise': {'price': 149.99, 'name': 'Enterprise Plan'}
}
    ...
    <body>
        <div class="checkout-container">
            ...
            <div class="plan-summary">
                <div class="plan-price">${plan_info['price']}<span style="font-size: 20px;">/month</span></div>
                <p style="margin-top: 15px; opacity: 0.9;">{plan_info['name']}</p>
            </div>
            ...
            <div class="payment-methods">
                ...
                <div class="payment-method" onclick="selectPayment('card')">
                    ...
                    <div>
                        ...
                        <p style="font-size: 14px; color: #666;">Visa, Mastercard, Amex</p>
                    </div>
                </div>
                ...
                <div class="payment-method" onclick="selectPayment('paypal')">
                    ...
                    <div>
                        ...
                        <p style="font-size: 14px; color: #666;">No account needed</p>
                    </div>
                </div>
            </div>
            ...
            <div id="cardPaymentForm" class="hidden">
                ...
                <button class="btn" onclick="processCardPayment(event)">Pay ${plan_info['price']}</button>
            </div>

            <!-- CashApp Payment (Stripe) -->
            <div id="cashappPaymentForm" class="hidden">
                <p style="text-align: center; margin: 20px 0;">You'll be redirected to Cash App to complete payment</p>
                <button class="btn" onclick="processCashAppPayment(event)">Continue to Cash App</button>
            </div>

            <!-- PayPal Payment -->
            <div id="paypalPaymentForm" class="hidden">
                <p style="text-align: center; margin: 20px 0;">You'll be redirected to PayPal (no account required)</p>
        ...
        <script>
            ...
            async function processCardPayment(e) {{
                ...
                try {{
                    ...
                    // Success!
                    window.location.href = '/payment-success?session_id=' + data.session_id;

                }} catch (error) {{
                    showError(error.message);
                    button.disabled = false;
                    button.textContent = 'Pay ${plan_info['price']}';
                }}
            }}
            ...
            async function processPayPalPayment(e) {{
                ...
                try {{
                    const response = await fetch('/paypal/create-payment', {{
                        method: 'POST',
                        headers: {{'Content-Type': 'application/json'}},
                        body: JSON.stringify({{
                            plan: '{plan}',
